...
$columns = isset($columns) ? $columns : array(); ?>
    ...
    <?php if (in_array('forum', $columns)) { ?>
        <td class="col-parent">
        	<?php echo $this->Html->link($topic['Forum']['title'], array('controller' => 'stations', 'action' => 'view', $topic['Forum']['slug'])); ?><br />
        </td>
    <?php } ?>
    <?php if (in_array('status', $columns)) { ?>
    ...
    <td class="col-activity">
        <?php if (!empty($topic['LastPost']['id'])) {
            echo $this->Forum->timeAgoInWords($topic['LastPost']['created'], array('timezone' => $this->Forum->timezone())); ?>
