        ...
        <div class="panel-body">
            <table class="table">
                <tbody>
                    ...
                    <tr class="<?php if ($isBuried) echo 'is-buried'; ?>" id="post-<?php echo $post_id; ?>">
                        <td class="post-time">
                            <?php echo $this->Html->tag('time',
                                $this->Forum->timeAgoInWords($post['Post']['created'], array('timezone' => $this->Forum->timezone())),
                                ...
                                if ($user) {
                                    ...
                                    if ($topic['Topic']['firstPost_id'] == $post_id) {
                                        if ($isMod || ($topic['Topic']['status'] && $user['id'] == $post['Post']['user_id'])) {
                                            $links[] = $this->Html->link('<span class="fa fa-wrench">' . __d('forum', 'Edit topic') . '</span>', array('controller' => 'topics', 'action' => 'edit', $topic['Topic']['slug'], (!empty($topic['Poll']['id']) ? 'poll' : '')), array('escape' => false, 'class' => 'js-tooltip', 'data-tooltip' => __d('forum', 'Edit Topic')));
                                        }

                                        if ($isMod) {
                                            $links[] = $this->Html->link('<span class="fa fa-times">' . __d('forum', 'Delete topic') . '</span>', array('controller' => 'topics', 'action' => 'delete', $topic['Topic']['slug']), array('escape' => false, 'confirm' => __d('forum', 'Are you sure you want to delete?'), 'class' => 'js-tooltip', 'data-tooltip' => __d('forum', 'Delete Topic')));
                                        }

                                        $links[] = $this->Html->link('<span class="fa fa-flag">' . __d('forum', 'Report topic') . '</span>', array('controller' => 'topics', 'action' => 'report', $topic['Topic']['slug']), array('escape' => false, 'class' => 'js-tooltip', 'data-tooltip' => __d('forum', 'Report Topic')));
                                    } else {
                                        if ($isMod || ($topic['Topic']['status'] && $user['id'] == $post['Post']['user_id'])) {
                                            $links[] = $this->Html->link('<span class="fa fa-wrench">' . __d('forum', 'Edit post') . '</span>', array('controller' => 'posts', 'action' => 'edit', $post_id), array('escape' => false, 'class' => 'js-tooltip', 'data-tooltip' => __d('forum', 'Edit Post')));
                                            $links[] = $this->Html->link('<span class="fa fa-times">' . __d('forum', 'Delete post') . '</span>', array('controller' => 'posts', 'action' => 'delete', $post_id), array('escape' => false, 'confirm' => __d('forum', 'Are you sure you want to delete?'), 'class' => 'js-tooltip', 'data-tooltip' => __d('forum', 'Delete Post')));
                                        }

                                        $links[] = $this->Html->link('<span class="fa fa-flag">' . __d('forum', 'Report post') . '</span>', array('controller' => 'posts', 'action' => 'report', $post_id), array('escape' => false, 'class' => 'js-tooltip', 'data-tooltip' => __d('forum', 'Report Post')));
                                    }

                                    if ($canReply) {
                                        $links[] = $this->Html->link('<span class="fa fa-quote-left">' . __d('forum', 'Reply to post') . '</span>', array('controller' => 'posts', 'action' => 'add', $topic['Topic']['slug'], $post_id), array('escape' => false, 'class' => 'js-tooltip', 'data-tooltip' => __d('forum', 'Quote')));
                                    }
                                }

                                $links[] = $this->Html->link('<span class="fa fa-link">' . __d('forum', 'Link to post') . '</span>', '#post-' . $post_id, array('escape' => false, 'class' => 'js-tooltip', 'target' => '_blank', 'data-tooltip' => __d('forum', 'Link To This')));
                                    ...
                                    <div id="post-ratings-<?php echo $post_id; ?>" class="post-ratings<?php if ($hasRated) echo ' has-rated'; ?>">
                                        <?php if (!$hasRated) { ?>
                                            <a href="javascript:;" onclick="return Forum.ratePost(<?php echo $post_id; ?>, 'up');" class="rate-up js-tooltip" data-tooltip="<?php echo __d('forum', 'Rate Up'); ?>">
                                                <span class="fa fa-arrow-up"><?php echo __d('forum', 'Rate Up'); ?></span>
                                            </a>
                                        <?php }
                                        ...
                                        if (!$hasRated) { ?>
                                            <a href="javascript:;" onclick="return Forum.ratePost(<?php echo $post_id; ?>, 'down');" class="rate-down js-tooltip" data-tooltip="<?php echo __d('forum', 'Rate Down'); ?>">
                                                <span class="fa fa-arrow-down"><?php echo __d('forum', 'Rate Down'); ?></span>
                                            </a>
                                        <?php } ?>
                                    </div>
                                <?php } ?>
                    ...
                    <tr>
                        <td class="span-3 post-author">
                            <h4 class="username"><?php echo $this->Html->link($post['User'][$userFields['username']], $this->Forum->profileUrl($post['User'])); ?></h4>
                            <?php if (!$isBuried) { ?>
								<?php
									if (!empty($post['User'][$userFields['totalPosts']])) {
										echo $this->Forum->getUserCountTag($post['User'][$userFields['totalPosts']]) . '<br />';
									}
								?>
								<br />
                                <?php echo $this->Forum->avatar($post) ?><br />
								<?php if (!empty($post['User'][$userFields['totalPosts']])): ?>
									<div class="data">
										<div class="label">Posts:</div>
										<div class="content"><?php echo $post['User'][$userFields['totalPosts']]; ?></div>
									</div> 
								<?php endif; ?>
                                <?php if (!empty($post['User'][$userFields['location']])): ?>
									<div class="data">
										<div class="label">Location:</div>
										<div class="content"><?php echo $post['User'][$userFields['location']]; ?></div>
									</div> 
								<?php endif;?>
                            <?php } ?>
                        </td>
    ...
    if ($settings['enableQuickReply'] && $canReply) { ?>
        ...
        <div id="quick-reply" class="panel quick-reply">
            ...
            <div class="panel-body">
                ...
                echo $this->element('wysiwyg-editor', array('id' => 'PostContent'));
